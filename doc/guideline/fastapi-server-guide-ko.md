...
5. 파일명이 `router`로 시작하면 `app/main.py`가 자동 탐색합니다.

## 수동 등록이 필요한 경우

다음과 같은 상황에서는 자동 탐색 대신 `app/main.py`의 `MANUAL_ROUTER_MODULES`에 모듈 경로를 직접 추가할 수 있습니다.

- 기존 파일명을 바로 바꾸기 어려운 경우
- 실험용 또는 단계적 전환 라우터를 한시적으로 붙여야 하는 경우
- `app/` 내부에 있지만 파일명이 `router*` 규칙을 따르지 않는 경우

예시:

```python
MANUAL_ROUTER_MODULES = (
    "app.experimental.special_routes",
    "app.legacy.admin_endpoints",
)
```

규칙:

- 값은 파일 경로가 아니라 파이썬 모듈 경로로 적습니다.
- 수동 등록 대상 모듈도 `router`라는 이름의 `APIRouter` 객체를 export해야 합니다.
- 자동 탐색 대상과 수동 등록 대상을 동시에 써도 되지만, 같은 모듈을 중복 등록할 필요는 없습니다.
- 장기적으로 유지할 라우터라면 가능하면 파일명을 `router.py`, `router_v1.py` 형태로 맞추는 편이 낫습니다.
...
- `MANUAL_ROUTER_MODULES`에 모듈 경로 대신 파일 시스템 경로를 적는 경우
  - 예: `app/legacy/router_extra.py`처럼 적으면 import되지 않습니다. `app.legacy.router_extra` 형태로 적어야 합니다.
...
3. 자동 탐색 예외가 필요하면 `MANUAL_ROUTER_MODULES`에 모듈 경로를 추가합니다.
4. 서버 기동 확인 후 필요한 엔드포인트를 직접 호출해 검증합니다.
5. 변경 내용을 문서와 팀 공지에 함께 반영합니다.
